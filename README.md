<h1 align="center">Certificadora de Competencia Específica</h1>

<p align="center">
Repositório utilizada no desenvolvimento do projeto requerido na matéria Certificadora de Competencia Específica
</p>

<div align="center">
  <figure>
    <img src="readME_image/Screenshot_3.png" alt="Protótipo Figma">
    <figcaption>Protótipo do website feito com o Figma - Tela Inicial</figcaption>
  </figure>
</div>

<br>

# Integrantes do grupo

<markdown-accessiblity-table data-catalyst="">
<table tabindex="0">
  <thead>
    <tr>
      <th align="center"><a href="https://github.com/Luis-Spessoto"><img src="https://avatars.githubusercontent.com/u/77413441?s=400&u=144e3f496c44706fe9f3d5b9be8c631a8044af71&v=4" alt="foto-Luis" width="110"><br><sub>Luís Felipe Spessoto</sub></a></th>
      <th align="center"><a href="https://github.com/BrunoBiazon"><img src="https://avatars.githubusercontent.com/u/184716758?v=4" alt="foto-Bruno" width="110"><br><sub>Bruno Circhia Biazon</sub></a></th>
      <th align="center"><a href="https://github.com/JoaoVFB"><img src="https://avatars.githubusercontent.com/u/187559847?v=4" alt="foto João" width="110"><br><sub>João Vitor Furquim</sub></a></th>
      <th align="center"><a href="https://github.com/DaniloFrazon"><img src="https://avatars.githubusercontent.com/u/187816067?v=4" alt="foto Danilo" width="110"><br><sub>Danilo Augusto</sub></a></th>
      <th align="center"><a href="https://github.com/Pedro-Meloo"><img src="https://avatars.githubusercontent.com/u/187815459?v=4" alt="foto Pedro" width="110"><br><sub>Pedro Henrique</sub></a></th>
    </tr>
  </thead>
</table>
</markdown-accessiblity-table>

# Descrição do projeto

O sistema consiste em uma plataforma web para gestão e acompafnhamento dos horários e da carga horária dos docentes do DACOM. O sistema utilizará os dados de horários já coletados pelo departamento para centralizar consultas, análises, indicadores e relatórios

# Tecnologias utilizadas

### Linguagens: JavaScript, TypeScript
### 	Frameworks: React, NodeJS, Express
### Ferramentas de Desenvolvimento: VS Code
### Banco de Dados: MongoDB
### 	Infraestrutura/Outros: GitHub (Repositório) e Figma (Prototipagem)
### 	Gerenciamento de processos/tarefas: Project/Board Kanban do GitHub 


# Divisão Laboral

- **Luís**: Responsável pela Prototipagem, UI/UX e PO
- **João**: Responsável pelo Banco de Dados e Integração
- **Danilo**: Responsável pelo FrontEnd
- **Pedro**: Responsável pelo BackEnd
- **Bruno**: Responsável pelo Backend 
